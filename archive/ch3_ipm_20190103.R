...
# unknown constant, set in "IPM: general parameters"
...
P_z1z <- function(z1, z, m.par_simple){
  return( s_z_simple(z, m.par_simple) 
...
F_z1z <- function(z1, z, m.par_simple){
  return( s_z_simple(z, m.par_simple) 
          * p_bz_simple(z, m.par_simple) 
          * b_z(z)
          * m.par_simple['p.r']
          * C_0z1(z1, m.par_simple) 
  )
}
...
# ---- IPM simple: lambda CI ** needs to be redone** ----
