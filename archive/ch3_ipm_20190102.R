...
# ---- generate recruits distribution ----
...
# reproduced: use ch 2 field regressions, see ch2 > expt2.4.1_field-reproduction_bodyweights_20181106.R
...
# probability reproduce? Pb(z)
dat$Reprod <- ''
dat$Reprod[dat$Surv == 0] <- NA
dat$Reprod[dat$Surv == 1] <-  predict(reprodyn.1_sizeonly # from ch 2
                                      , newdata = data.frame(vol = dat$v[dat$Surv == 1] )
                                      , type = 'response'
)
  
  
# fecundity metric b(z)
dat$Offspring <- ''
...
### define F component of kernel ("reproduction", really just recruitment): F(z') = C_0(z')

F_z1 <- function(z1, m.par){
  return( C_0z1(z1, m.par) )
}
