...
s_z_simple <- function(z, m.par_simple){
  ...
  # linear predictor
  linear.p <- m.par_simple['surv.int'] + m.par_simple['surv.z'] * z
  
  p <- ifelse(z < 8 # condition
              , 1/(1+exp(-linear.p)) # if true (back transform from logit)
              , 0 # if false
  )
  #p <- 1/(1+exp(-linear.p))
  return( unname(p) )
...
G_z1z_simple <- function(z1, z, m.par_simple){
  ...
  mu <- m.par_simple['grow.int'] + m.par_simple['grow.z'] * z # mean size next year
...
zvals <- seq(0, 8, length = 400)
g1 <- G_z1z_simple(zvals, min(zvals), m.par_simple) # small individuals
g2 <- G_z1z_simple(zvals, mean(zvals), m.par_simple) # medium individuals
g3 <- G_z1z_simple(zvals, max(zvals), m.par_simple) # large individuals
...
WrongPlace <- function(z, U) {
  fac1 <- s_z_simple(z, m.par_simple)
  fac2 <- integrate(function(x) G_z1z_simple(x, z, m.par_simple), U, Inf)$value
  return(fac1 * fac2)
}
zvals <- Wvals <- seq(0, 8, length = 400)
for (j in seq_along(zvals)) Wvals[j] <- WrongPlace(zvals[j], 8)
...
# U = 8.0 = upper bound; maximum observed size in field is 7.637 mm operc length
...
mk_K <- function(m, m.par, L, U) {
  ...
  K <- P # IPM kernel only has survival and growth; add recruits in later
...
IPM_simple <- mk_K(100, m.par_simple, 0.1, 8)
...
plot(stable.z.dist_simple/diff(meshpts_simple)[1]  ~ meshpts_simple) #from Monocarp Calculations.R
