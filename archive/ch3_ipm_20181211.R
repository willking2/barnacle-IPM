...
mean(rec$operc_length_mm)
sd(rec$operc_length_mm)
median(rec$operc_length_mm) # mean > median, slightly right skewed

recruit.x <- seq(0, 3, length = 100)

recruit.Gaussian <- dnorm(recruit.x
                          , m = mean(rec$operc_length_mm)
                          , sd = sd(rec$operc_length_mm)
)
lines(recruit.x
      , recruit.Gaussian
)

gammafit <- fitdist(data = rec$operc_length_mm
                    , distr = 'gamma'
                    , method = 'mle'
)
